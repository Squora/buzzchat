...
#[Route('/api/v1/users/me/status', name: 'api_v1_users_update_status', methods: ['PATCH'])]
final class UpdateOnlineStatusHandler extends AbstractController
{
    ...
    public function __invoke(Request $request): JsonResponse
    {
        ...
        try {
            $dto = $this->serializer->deserialize(
                $request->getContent(),
                UpdateOnlineStatusRequest::class,
                'json'
            );
        } catch (\Throwable) {
            return new JsonResponse(
        ...
        try {
            $user = $this->userService->updateOnlineStatus($currentUser, $dto);
